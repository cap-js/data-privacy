...
module.exports = class DPIPlugin extends cds.build.Plugin {
  ...
  async build() {
    ...
    await this.copy(path.join(__dirname, 'drm-service.js')).to('drm-service.js');
    await this.copy(path.join(__dirname, 'utils.js')).to('utils.js');
    await this.copy(path.join(__dirname, 'drm-handlers')).to('drm-handlers'); //REVISIT: only copy .js files
    await this.copy(path.join(__dirname, 'model')).to('model');
  }

  async updateRetentionConfig(csn) {
    const result = getDPIentities(csn, csn.definitions.DRMService);
    ...
    if (hasMta) {
      ...
      if (dpiRetentionInstance && dpiRetentionInstance.parameters?.config?.dataPrivacyConfiguration?.retentionConfiguration) {
        dpiRetentionInstance.parameters.config.dataPrivacyConfiguration.retentionConfiguration.dataSubjectRoles = result.dataSubjectRoles;
        dpiRetentionInstance.parameters.config.dataPrivacyConfiguration.retentionConfiguration.organizationAttributes = result.organizationAttributes;
      }
