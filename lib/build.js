...
const { translationUtils, _getLegalEntity } = require('./utils');
const dpiSrvGeneration = require('./dpiSrvGeneration');
const UTF_8 = 'utf-8'
const WARNING = "Warning", ERROR = "Error", INFO = "Info";
 ...
 */
module.exports = class DataPrivacyIntegrationBuildPlugin extends BuildPlugin {
    ...
    getRetentionConfigs(csn) {
        ...
        for (let each in csn.definitions) {
            ...
            if (!def.query && !def.projection && def.kind === 'entity' && def['@PersonalData.EntitySemantics'] === 'DataSubject') {
                ...
                const dataSubject = {
                    ...
                    dataSubjectDeletionEndPoint: "/drm/dataSubjectBlocking",
        ...
        return {
            applicationName: cds.env.requires?.dpi?.name || this.service.name,
