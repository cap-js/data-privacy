...
module.exports = class DPIPlugin extends cds.build.Plugin {
  ...
  async updateRetentionConfig(csn) {
    ...
    const existingConfig = await fs.readFile(path.join(cds.root, 'dpi-retention.json'))
    const dpConfig = {
      dataPrivacyConfiguration: {
        retentionConfiguration: {
          applicationConfiguration: {
            ...
            iLMObjectDiscoveryEndPoint: "/drm/iLMObjects"
          },
          dataSubjectRoles: result.dataSubjectRoles,
          organizationAttributes: result.organizationAttributes
        }
      }
    }
    await fs.writeFile(path.join(cds.root, 'dpi-retention.json'), Object.assign(existingConfig ?? {}, dpConfig))
  }
