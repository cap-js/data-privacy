...
function serveDataSubjectEligibleForDeletion(srv, db) {
    ...
    function whereClauseForRetentionSets(referenceDates) {
        ...
        const result = {
            wheresWithCondition: wheresWithCondition.map(w => ({xpr: w})).concat('or'),
            wheresWithNegConditions: wheresWithNegConditions.map(w => ({xpr: w})).concat('or')
        }
        LOG.debug('Wheres with condition: ', result.wheresWithCondition)
        LOG.debug('Wheres with inverted condition: ', result.wheresWithNegConditions)
        return result;
    }
}
