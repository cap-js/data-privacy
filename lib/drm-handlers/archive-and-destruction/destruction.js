...
function serveDestructionRequests(srv, db) {
    ...
    srv.on('destruction', async req => {
        const {
            applicationName,
            runId: requestId, iLMObjectName: legalGround, selectionCriteria = []
        } = req.data

        LOG.info(`Requested destruction for legal ground: ${legalGround} and app ${applicationName} with`,
    ...
    srv.on('simulateDestruction', async req => {
        ...
        LOG.info(`Requested simulateDestruction for legal ground: ${legalGround} and app ${applicationName} with`,
            ` the request ID ${requestId}.`,
            `The selection criteria are:`, selectionCriteria)
