...
const { _getEntityForILMObject } = require('../../utils')
...
function serveDestructionRequests(srv, db) {
    ...
    srv.on('destruction', async req => {
        ...
        LOG.info(`Requested destruction for iLMObject: ${iLMObjectName} and app ${applicationName} with`,
            ` the request ID ${runId}.`,
            `The selection criteria are:`, selectionCriteria)
            
        const iLMObjectsToBeDestroyedIDs = await getILMObjectIDsToBeDeleted(runId, iLMObjectName, selectionCriteria)
       
        if (iLMObjectsToBeDestroyedIDs.length > 0) {
            LOG.info(`Deleted the following blocking store entries:`, iLMObjectsToBeDestroyedIDs)
            await DELETE.from(BlockingStore).where({ID: {in: iLMObjectsToBeDestroyedIDs}})
        }
        req.res.statusCode = 202
        LOG.info(`Result of destruction request: `, "Request intiated succesfully.", "with status code 4",
            `The request ID is ${runId}.`)
        return {
            requestId: runId,
            requestStatusCode: 4, //1 in progress, 4 completed, 5 failed
    ...
    srv.on('simulateDestruction', async req => {
        ...
        LOG.info(`Requested simulateDestruction for iLMObject: ${iLMObjectName} and app ${applicationName} with`,
            ` the request ID ${runId}.`,
            `The selection criteria are:`, selectionCriteria)

        const iLMObjectsToBeDestroyedIDs = await getILMObjectIDsToBeDeleted(runId, iLMObjectName, selectionCriteria)
        req.res.statusCode = 202
        LOG.info(`Result of simulated destruction request: `, "Request intiated succesfully.", "with status code 4",
            `The request ID is ${runId}. Destroyed: `, iLMObjectsToBeDestroyedIDs)
        return {
            requestId: runId,
            requestStatusCode: 4,
    ...
    async function getILMObjectIDsToBeDeleted(runId, iLMObjectName, selectionCriteria) {
