...
const LOG = cds._dpi.log('data-privacy-information');
...
const exposeCompositionsWithRewrites = (fullName, parentDefinition, dsFields, m, redirectForParent = false, PDMEntities) => {
  ...
  if (compositions.length > 0) {
    ...
    for (const [name, comp] of compositions) {
      ...
      const informationEntity = m.definitions['DPIInformationService' + '.' + entityName] ?? JSON.parse(JSON.stringify(entity));
      ...
      if (entityAlreadyExposed) {
        ...
        result['DPIInformationService.' + entityName] = informationEntity;
      }

      //REVISIT - does not add deeper than 1 level as added backlinks cannot be referenced in deeper comps 
      Object.assign(result, exposeCompositionsWithRewrites(comp.target, informationEntity, newDsFields, m, !hasBacklink(entity.elements, fullName), PDMEntities));
    }
  }
  return result
}
