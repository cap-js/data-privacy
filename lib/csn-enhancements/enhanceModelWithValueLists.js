...
module.exports = function enhanceModelWithValueLists(entityName, entity, model) {
  const options = {
    retentionService: model.definitions['sap.dpp.RetentionService'],
    entityName,
    entity,
    model
  }
...
/**
 * Adds a value help view to the sap.dpp.RetentionService to act as the value help endpoint. 
...
function handleValueList(value, name, type = 'selection', o) {
  ...
  const keyFieldName = type === 'condition' ? 'conditionFieldValue' : (type === 'orgAttribute' ? 'organizationAttributeValue' : 'value');
  const descriptionFieldName = type === 'condition' ? 'conditionFieldValueDescription' : (type === 'orgAttribute' ? 'organizationAttributeValueDescription' : 'valueDescription');
  const conditionE = JSON.parse(JSON.stringify(conditionEntity));
  conditionE.elements = {
    [keyFieldName]: {
      key: true,
      type: "cds.String",
  ...
  let VHName = null;
  if (Object.keys(value).some(p => p.startsWith('@Common.ValueList') && p.endsWith('.CollectionPath'))) {
    const valueHelps = Object.keys(value).filter(p => p.startsWith('@Common.ValueList') && p.endsWith('.CollectionPath'));
    LOG.debug(`Using ${valueHelps[0]} for the value Help of type ${type} for ${name} on entity ${o.entityName}.`)
    const vHIdentifier = valueHelps[0].substring(0, valueHelps[0].length-15);
    const valueField = value[vHIdentifier+'.Parameters'] ? value[vHIdentifier+'.Parameters'].find(param => param['$Type'] === 'Common.ValueListParameterInOut' || param['$Type'] === 'Common.ValueListParameterOut')?.ValueListProperty : Object.entries(o.entity.keys)[0][0]
    const valueDescField = value[vHIdentifier+'.Parameters'] ? value[vHIdentifier+'.Parameters'].filter(param => param['$Type'] === 'Common.ValueListParameterDisplayOnly').reduce((acc, val) => {
    ...
    const effectiveModel = cds.reflect(JSON.parse(JSON.stringify(o.model)))
    ...
    const targetEntity =
      ...
      ?? effectiveModel[o.entityName].elements[effectiveModel[o.entityName].elements[name]._foreignKey4]?._target;
    if (!targetEntity) {
      LOG.error(
    ...
    conditionE.query = {
      SELECT: {
        ...
        columns: [
          {key: true, ref: [valueField], as: keyFieldName, cast: {type: 'cds.String'}},
          valueDescField === 'null' ? {xpr: [{val: ''}], as: descriptionFieldName, cast: {type: 'cds.String'}} : {ref: [valueDescField], as: descriptionFieldName, cast: {type: 'cds.String'}}
        ]
      }
    };
    value['@ILM.ValueHelp.Id'] = value['@Common.ValueList.CollectionPath'];
  } else if (value.type === 'cds.Association' || value.type === 'cds.Composition') { //Case of missing ValueList annotation -> But its a relation
    ...
    let descriptionField = target.elements[keys[0]]['@Common.Text']?.['='] ?? target['@UI.HeaderInfo.Title.Value']?.['='] ?? 'null';
    conditionE.query = {
      SELECT: {
        distinct: true,
        from: { ref: [value.target] },
        columns: [
          {key: true, ref: [keys[0]], as: keyFieldName, cast: {type: 'cds.String'}},
          descriptionField === 'null' ? {xpr: [{val: ''}], as: descriptionFieldName, cast: {type: 'cds.String'}} : {ref: descriptionField.split('.'), as: descriptionFieldName, cast: {type: 'cds.String'}}
        ]
      }
    };
    ...
    conditionE.query = {
      SELECT: {
        ...
        columns: [
          {key: true, ref: [name], as: keyFieldName, cast: {type: 'cds.String'}},
          descriptionField === 'null' ? {xpr: [{val: ''}], as: descriptionFieldName, cast: {type: 'cds.String'}} : {ref: descriptionField.split('.'), as: descriptionFieldName, cast: {type: 'cds.String'}}
        ]
      }
    };
  ...
  conditionE[`@ILM.ValueHelp.Path`] = `${o.retentionService.path ?? o.retentionService['@path']}/${VHName}`;
  value[`@ILM.ValueHelp.Path`] = `${o.retentionService.path ?? o.retentionService['@path']}/${VHName}`;
  value['@ILM.ValueHelp.Type'] = type;
  value['@ILM.ValueHelp.Entity'] = `${'sap.dpp.RetentionService'}.${VHName}`;
  o.model.definitions[`${'sap.dpp.RetentionService'}.${VHName}`] = conditionE;
  return conditionE;
}
...
function buildConditions(o) {
  for (const elementName in o.entity.elements) {
    ...
    if (element['@PersonalData.FieldSemantics'] === 'PurposeID' || element['@ILM.FieldSemantics'] === 'ProcessOrganizationAttribute') {
...
function buildSelectionCriteria(o) {
  ...
  for (const elementName in o.entity.elements) {
    ...
    if (type && fieldIsAllowedForFiltering(element, o.entity)) {
...
function buildOrganizationAttribute(o) {
  ...
  if (orgAttributes.length > 1) {
    return LOG.error(`${o.entityName} has multiple organization attributes configured (annotated with @PersonalData.FieldSemantics : 'DataControllerID' or @ILM.FieldSemantics : 'LineOrganizationAttribute'! Only one attribute is allowed!`)
  }
  if (orgAttributes.length === 0) {
    return LOG.error(`${o.entityName} has no organization attributes configured (annotated with @PersonalData.FieldSemantics : 'DataControllerID' or @ILM.FieldSemantics : 'LineOrganizationAttribute'! One attribute is required to configure the entity as an ILMObject for Data Privacy Retention Management!`)
  }
  const orgAttributeName = orgAttributes[0];
  const orgAttributeDefinition = handleValueList(o.entity.elements[orgAttributeName], orgAttributeName, 'orgAttribute', o)
  
  const existingOrgAttributes = Object.keys(o.model.definitions).filter(n => n.startsWith(`sap.dpp.RetentionService.valueHelp_orgAttribute`)).map(def => o.model.definitions[def]);
  if (existingOrgAttributes.some(o => o['@ILM.OrganizationAttributeName'] === orgAttributeName && o['@ILM.ValueHelpPath'] !== orgAttributeDefinition['@ILM.ValueHelpPath'])) {
    const existingAttribute = existingOrgAttributes.find(o => o.organizationAttributeName === orgAttributeName && o.organizationAttributeValueHelpEndPoint !== orgAttributeDefinition['@ILM.ValueHelpPath'])
    LOG.warn(`Organisational attributes require unique organizationAttributeName properties! Tried to register another attribute for the name ${orgAttributeName} now from ${o.entity.name} with the generated value help endpoint ${orgAttributeDefinition['@ILM.ValueHelpPath']} deviating from the existing registered attribute with the value help endpoint ${existingAttribute['@ILM.ValueHelpPath']}`);
  }
  orgAttributeDefinition['@ILM.OrganizationAttributeName'] = o.entity.elements[orgAttributeName]['@ILM.ValueHelp.Id'];
