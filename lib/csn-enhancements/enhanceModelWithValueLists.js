...
const { _getRoot, mapCDStoDRMtype } = require('../utils');
...
module.exports = function enhanceModelWithValueLists(entityName, entity, model) {
  const options = {
    retentionService: model.definitions['DPIRetentionService'],
    entityName,
    entity,
    model
  }
...
/**
 * Adds a value help view to the DPIRetentionService to act as the value help endpoint. 
...
function handleValueList(value, name, type = 'selection', o) {
  ...
  if (value['@Common.ValueList.CollectionPath'] && value.type !== 'cds.Association' && value.type !== 'cds.Composition') {
    const valueField = value['@Common.ValueList.Parameters'] ? value['@Common.ValueList.Parameters'].find(param => param['$Type'] === 'Common.ValueListParameterInOut' || param['$Type'] === 'Common.ValueListParameterOut')?.ValueListProperty : Object.entries(o.entity.keys)[0][0]
    ...
    VHName = `valueHelp_${type}_${o.entityName}_${value.keys ? name + '_' + value.keys[0].ref[0] : name}`;
    const rootEntity = _getRoot(o.entity);
    const namespace = rootEntity.name.substring(0, rootEntity.name.length - nameOf(rootEntity).length - 1)
    const targetEntity =
      o.model.definitions[`${namespace}.${value['@Common.ValueList.CollectionPath']}`]
      ?? o.effectiveCSN[o.entityName].elements[o.effectiveCSN[o.entityName].elements[name]._foreignKey4]?._target;
    if (!targetEntity) {
      LOG.error(
    ...
    conditionE.query = {
      SELECT: {
        from: { ref: [targetEntity] },
        columns: [
          {key: true, ref: [valueField], as: keyFieldName, cast: {type: 'cds.String'}},
          valueDescField === 'null' ? {xpr: ['null'], as: descriptionFieldName, cast: {type: 'cds.String'}} : {ref: [valueDescField], as: descriptionFieldName, cast: {type: 'cds.String'}}
        ]
      }
    };
    ...
    conditionE.query = {
      SELECT: {
        ...
        columns: [
          ...
          descriptionField === 'null' ? {xpr: ['null'], as: descriptionFieldName, cast: {type: 'cds.String'}} : {ref: descriptionField.split('.'), as: descriptionFieldName, cast: {type: 'cds.String'}}
        ]
      }
    };
  } else {
  ...
  value['@ILM.ValueHelp.Entity'] = `${'DPIRetentionService'}.${VHName}`;
  o.model.definitions[`${'DPIRetentionService'}.${VHName}`] = conditionE;
  return conditionE;
}
...
function buildOrganizationAttribute(o) {
  ...
  const existingOrgAttributes = Object.keys(o.model.definitions).filter(n => n.startsWith(`DPIRetentionService.valueHelp_orgAttribute`)).map(def => o.model.definitions[def]);
  ...
  if (o.entity.elements[orgAttributeName].keys) {
    orgAttributeDefinition['@ILM.OrganizationAttributeName'] = orgAttributeName + '_' + o.entity.elements[orgAttributeName].keys[0].ref[0];
  } else {
    orgAttributeDefinition['@ILM.OrganizationAttributeName'] = orgAttributeName;
  }
