...
function handleValueList(value, name, type = 'selection', o) {
  ...
  value['@ILM.ValueHelp.Entity'] = `${o.retentionService.name}.${VHName}`;
...
function buildOrganizationAttribute(o) {
  ...
  if (orgAttributes.length > 1) {
    return this.pushMessages(`${o.entityName} has multiple organization attributes configured (annotated with @PersonalData.FieldSemantics : 'DataControllerID' or @ILM.FieldSemantics : 'LineOrganizationAttribute'! Only one attribute is allowed!`, `ERROR`)
  }
  if (orgAttributes.length === 0) {
    return this.pushMessages(`${o.entityName} has no organization attributes configured (annotated with @PersonalData.FieldSemantics : 'DataControllerID' or @ILM.FieldSemantics : 'LineOrganizationAttribute'! One attribute is required to configure the entity as an ILMObject for Data Privacy Retention Management!`, `ERROR`)
  }
