  ...
  for (let each in inferredCSN.definitions) {
    ...
    for (let ele in inferredDef.elements) {
      ...
      if (def.elements) {
        ...
        if (inferredEleDef["@Communication.IsEmailAddress"]) {
          isPersonal(eleDef);
        } else if (inferredEleDef["@Communication.IsPhoneNumber"]) {
          isPersonal(eleDef);
        }
      }
    }
    ...
    if (inferredDef["@Communication.Contact.email"]) {
      for (const email of inferredDef["@Communication.Contact.email"]) {
        ...
        if (def.elements && email.address["="] && inferredDef.elements[email.address["="]]) {
      ...
      for (const blockingAnnotation of blockingAnnotations) {
        if (blockingAnnotation === "@ILM.BlockingEnabled.$edmJson.$Path") {
          ...
          def["@ILM.BlockingEnabled"] = {
            ...
            _query: SELECT.one.from(service + "." + segments[0]).columns(segments[1]),
            _service: service
          };
          break;
        } else if (
          ...
          try {
            ...
            // REVISIT: Use xpr once cds compiler supports sub selects in expressions
            def["@ILM.BlockingEnabled"] = { _query: xpr };
          } catch (error) {
            LOG.error(
