...
function entityValidation(entityName, m) {
  ...
  if (entity['@PersonalData.EntitySemantics'] === 'DataSubject') {
    for (const k in entity.elements) {
      ...
      if (
        ...
        LOG.warn(
          `The composition ${k} of the data subject ${entityName} points to ${element.target}. However ${element.target} is not annotated with '@PersonalData.EntitySemantics': 'DataSubjectDetails'.`,
        );
      }
    }
  }
  ...
  for (const k in entity.elements) {
    ...
    if (
      ...
      entity.elements[k]['@ILM.FieldSemantics'] === 'LineOrganizationAttribute'
    ) {
      LOG.error(
        `${k} of ${entityName} is annotated with @ILM.FieldSemantics : 'LineOrganizationAttribute' but also a conflicting @PersonalData.FieldSemantics annotation! Please remove either of them!`,
      );
    }
    if (
      entity.elements[k]['@PersonalData.FieldSemantics'] !== 'PurposeID' &&
      entity.elements[k]['@ILM.FieldSemantics'] === 'ProcessOrganizationAttribute'
    ) {
      LOG.error(
        `${k} of ${entityName} is annotated with @ILM.FieldSemantics : 'ProcessOrganizationAttribute' but also a conflicting @PersonalData.FieldSemantics annotation! Please remove either of them!`,
      );
    }
...
const ilmFieldsForReference = {
  LineOrganizationAttribute: 1,
  ProcessOrganizationAttribute: 1,
};
