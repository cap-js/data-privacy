...
function entityValidation(entityName, m, DPI_SERVICES) {
  ...
  if (
    entity["@ILM.ObjectName"] !== undefined &&
    entity["@ILM.ObjectName"] !== null &&
    typeof entity["@ILM.ObjectName"] !== "string"
  ) {
    LOG.error(`Only string values are allowed for @ILM.ObjectName on ${entityName}!`);
  }
  if (
    typeof entity["@ILM.ObjectName"] === "string" &&
    entity["@PersonalData.EntitySemantics"] !== "Other" &&
  ...
  // Validate @Auditing.AuditorScopes values are ASCII word characters only
  if (entity["@Auditing.AuditorScopes"]) {
    for (const scope of entity["@Auditing.AuditorScopes"]) {
      if (!/^\w+$/.test(scope)) {
        LOG.error(
          `Invalid @Auditing.AuditorScopes value "${scope}" on ${entityName}. Only ASCII word characters (a-z, A-Z, 0-9, _) are allowed!`
        );
      }
    }
  }
...
function firstEntityWithAnnotation(entityName, annotation, model) {
  ...
  const sourceEntity = _getSourceEntity(_getQueryFrom(entity));
  if (!sourceEntity) {
    return true;
  }
  const parent = model.definitions[sourceEntity];
...
function firstElementWithAnnotation(entityName, elementName, annotation, model) {
  ...
  const parentElementRef = cqn?.columns?.find((c) => c.ref && c.as && c.as === elementName)
    ?.ref ?? [elementName];

  let currentEntity = model.definitions[sourceEntity];
