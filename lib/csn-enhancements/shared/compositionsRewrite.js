...
function buildColumns({
  ...
  additionalFields.push(
    ...Object.entries(parentDefinition.elements)
      ...
      .map(([n]) => {
        let r = formatter(n);
        // Clone the parent's element instead of taking a shared reference — a shared
        // reference would incorrectly propagate `key: true`
        const clonedElement = structuredClone(parentDefinition.elements[n]);
        delete clonedElement.key;
        newEntity.elements[r.as] = clonedElement;
        if (n === "ID") {
  ...
  if (redirectForParent && entity.elements[backLinkName] && entity.elements[backLinkName].keys) {
    entity.elements[backLinkName].keys.forEach((k) => {
      ...
      if (!additionalFields.some((a) => a === f)) {
        ...
        // Clone to avoid propagating parent's `key: true` to this foreign-key column
        const clonedElement = structuredClone(parentDefinition.elements[k.ref[0]]);
        delete clonedElement.key;
        newEntity.elements[f.as] = clonedElement;
      }
    });
  }
