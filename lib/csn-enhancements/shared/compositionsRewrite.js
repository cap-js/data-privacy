...
const hasBacklink = (elements, parent) =>
  Object.values(elements).some((e) => e.target && e.target === parent);
const backlink = (elements, parent) =>
  Object.entries(elements).find(([, e]) => e.target && e.target === parent)[0];
...
const compCondition = (name, cmp, backLinkName) => {
  if (cmp.on && backLinkName) {
    return structuredClone(cmp.on).reduce((acc, val) => {
      if (val.ref) {
        if (val.ref[0] === name) {
          val.ref.shift();
        } else {
    ...
    return structuredClone(cmp.keys).reduce((acc, val) => {
      if (acc.length > 0) {
        acc.push('and');
      }
      acc.push(
...
const exposeCompositionsWithRewrites = (
  fullName,
  parentDefinition,
  m,
  o = {
  ...
  if (compositions.length > 0) {
    ...
    for (const name of compositions) {
      const comp = resolveCustomElementType(parentDefinition.elements[name], m);
      ...
      let backLinkName = hasBacklink(entity.elements, fullName)
        ? backlink(entity.elements, fullName)
        : 'backlink';
      ...
      const newEntity =
        m.definitions[o.dppServiceName + '.' + entityName] ?? structuredClone(entity);
      ...
      if (entityAlreadyExposed) {
          ...
          if (!hasBacklink(entity.elements, fullName)) {
            ...
            for (const [cname, child] of children) {
              ...
              additionalFields[cname] = {
                ...
                target: child.target,
                cardinality:
                  child.cardinality && child.cardinality.max == '1' ? { max: 1 } : { max: '*' },
        ...
        function columns() {
          ...
          for (const field in o.dsFields) {
            ...
            //last check in case managed association is such a field already
              ...
              newEntity.elements[newField.as] =
                parentDefinition.elements[o.dsFields[field].replaceAll('.', '_')];
              additionalFields.push(newField);
              newDsFields[field] =
                !o.redirectForParent && entity.elements[backLinkName]
                  ? `${backLinkName}.${o.dsFields[field]}`
                  : `${backLinkName}_${o.dsFields[field]}`;
            }
          }
          ...
          mixinFieldsToExclude.forEach((f) => {
            if (
              !additionalFields.some((addField) => addField.ref && addField.ref.join('.') === f)
            ) {
      ...
      Object.assign(
        ...
        exposeCompositionsWithRewrites(comp.target, newEntity, m, {
          ...
          redirectForParent: !hasBacklink(entity.elements, fullName),
          entities: o.entities,
