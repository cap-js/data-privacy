...
function fixRelationTarget(def, exposedEntities, serviceName, model) {
  for (const ele in def.elements) {
    ...
    } else if (def.elements[ele].key) {
      // Add key explicitly to projection else it gets lost as a key field
      const query = def.query?.SELECT ?? def.projection;
      // i18n-files for example has no query and would else crash
      if (!query) continue;
      query.columns ??= ["*"];
      if (!query.columns.some((c) => (c.ref && c.ref.length === 1 && c.ref[0] === ele) || (c.as && c.as === ele))) {
        query.columns.push({
          ref: [ele],
          key: true
        });
