const { _getSourceEntity, _getQueryFrom } = require("../../utils");
...
function fixRelationTarget(def, exposedEntities, serviceName, model) {
  for (const ele in def.elements) {
    ...
    if (def.elements[ele].target && typeof exposedEntities[def.elements[ele].target] === "string") {
      ...
      typeof findExposedEntityForTarget(def.elements[ele].target, exposedEntities, model) ===
        "string"
    ) {
      def.elements[ele].target = findExposedEntityForTarget(
        def.elements[ele].target,
        exposedEntities,
        model
      );
