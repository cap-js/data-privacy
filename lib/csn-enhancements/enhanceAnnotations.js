...
module.exports = function annotationHelper(m) {
    ...
    for (let each in m.definitions) {
        ...
        if (Object.keys(def).filter(k => k.startsWith('@ILM.BlockingEnabled')).length > 0) {
            const blockingAnnotations = Object.keys(def).filter(k => k.startsWith('@ILM.BlockingEnabled'));
                ...
                if (blockingAnnotation === '@ILM.BlockingEnabled.$edmJson.$Path') {
                    ...
                    if (segments[0].endsWith('EntityContainer')) {
                        ...
                        for (const segment of nameSegments) {
                            ...
                            if (m.definitions[cumulatedSegments]?.type === 'service' && m.definitions[cumulatedSegments + '.' + segments[0]]) {
                    ...
                    if (!singletonTarget) {
                        return LOG.error(`${each} is annotated with @ILM.BlockingEnabled, however the entity path given cannot be found! You can either write @ILM.IsBlockingEnabled: {$edmJson: {$Path: '<Service name>.EntityContainer/<singleton entity>/<property>'}} or @ILM.IsBlockingEnabled: 'SELECT <property> FROM <singleton>'`)
                    }
                    if (!singletonTarget['@odata.singleton']) {
                        return LOG.error(`${each} is annotated with @ILM.BlockingEnabled, however the dynamic target path points to ${service + '.' + segments[0]} which is not annotated with @odata.singleton!`)
                    }
                    const targetedProperty = singletonTarget.elements[segments[1]];
                    if (!targetedProperty) {
                        return LOG.error(`${each} is annotated with @ILM.BlockingEnabled, however the dynamic target path points to '${segments[1]}' on ${service + '.' + segments[0]} and '${segments[1]}' is not a property on the singleton!`)
                    }
                    delete def[blockingAnnotation];
                    def['@ILM.BlockingEnabled'] = {xpr: [SELECT.one.from(service + '.' + segments[0]).columns(segments[1])], _service: service}
                    break;
                } else if (blockingAnnotation === '@ILM.BlockingEnabled' && typeof def[blockingAnnotation] === 'string') {
                    try {
                        const xpr = cds.parse.expr(def[blockingAnnotation]);
                        if (!xpr.SELECT) {
                            return LOG.error(`${each} is annotated with @ILM.BlockingEnabled, however the expression cannot be used. The expression must follow the pattern: @ILM.IsBlockingEnabled: 'SELECT <property> FROM <singleton>'`)
                        }
                        def['@ILM.BlockingEnabled'] = {xpr: [xpr]}
                    } catch (error) {
                        return LOG.error(`${each} is annotated with @ILM.BlockingEnabled, however the annotation value could not be parsed as an expression! Parsing error: ${error}`)
                    }
                } else if (blockingAnnotation.startsWith('@ILM.BlockingEnabled.$edmJson')) {
                    return LOG.error(`${each} is annotated with @ILM.BlockingEnabled, however the path is invalid! You can either write @ILM.IsBlockingEnabled: {$edmJson: {$Path: '<Service name>.EntityContainer/<singleton entity>/<property>'}} or @ILM.IsBlockingEnabled: 'SELECT <property> FROM <singleton>'`)
                }
            }
        }
    }
