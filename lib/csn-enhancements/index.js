...
module.exports = function enhanceModel(m) {
  ...
  if (m.meta?.[_enhanced] || m.definitions['sap.ilm.RetentionService']?.['@sap.ilm.model.enhanced']) return // already enhanced
  ...
  const autoFillDPIInformationSrv = cds.env.requires['sap.ilm.InformationService'].model === '@sap/cds-dpi/srv/DPIInformation';
  const autoFillDPIRetentionSrv = cds.env.requires['sap.ilm.RetentionService'].model.startsWith('@sap/cds-dpi/srv/');
  ...
  for (const each in m.definitions) {
    ...
    if (entity in RetentionEntities || entity in InformationEntities || entity.startsWith('sap.ilm.RetentionService') || entity.startsWith('sap.ilm.InformationService')) {
      ...
      if (_getBlockingDateField(def.elements) && !_getBlockingDateField(originalEntity.elements)) {
        originalEntity.elements[_getBlockingDateField(m.definitions['sap.ilm.blocking'].elements)] = m.definitions['sap.ilm.blocking'].elements[_getBlockingDateField(m.definitions['sap.ilm.blocking'].elements)]
        const query = originalEntity.query?.SELECT ?? originalEntity.projection;
        if (query) {
          const queryTarget = m.definitions[query.from?.ref[0]]
          query.columns ??= ['*']
          if (!query.columns.includes('*') && !query.columns.some(c => c.ref && c.ref[0] === _getBlockingDateField(queryTarget.elements))) {
            query.columns.push({ ref: [_getBlockingDateField(def.elements)] });
          }
        }
      }
      if (_getEndOfRetentionField(def.elements) && !_getEndOfRetentionField(originalEntity.elements)) {
        originalEntity.elements[_getEndOfRetentionField(m.definitions['sap.ilm.destruction'].elements)] = m.definitions['sap.ilm.destruction'].elements[_getEndOfRetentionField(m.definitions['sap.ilm.destruction'].elements)]
        const query = originalEntity.query?.SELECT ?? originalEntity.projection;
        if (query) {
          const queryTarget = m.definitions[query.from?.ref[0]]
          query.columns ??= ['*']
          if (!query.columns.includes('*') && !query.columns.some(c => c.ref && c.ref[0] === _getEndOfRetentionField(queryTarget.elements))) {
    ...
    if (autoFillDPIInformationSrv) {
      ...
      const informationEntity = m.definitions['sap.ilm.InformationService' + '.' + entityName] ?? JSON.parse(JSON.stringify(def));
    ...
    if (autoFillDPIRetentionSrv) {
      ...
      if (!def.query && !def.projection) {
        if (!_getBlockingDateField(def.elements) && !_getEndOfRetentionField(def.elements)) {
          ...
          def.elements[_getBlockingDateField(m.definitions['sap.ilm.blocking'].elements)] = m.definitions['sap.ilm.blocking'].elements[_getBlockingDateField(m.definitions['sap.ilm.blocking'].elements)]
        } else if (!_getEndOfRetentionField(def.elements)) {
          def.elements[_getEndOfRetentionField(m.definitions['sap.ilm.destruction'].elements)] = m.definitions['sap.ilm.destruction'].elements[_getEndOfRetentionField(m.definitions['sap.ilm.destruction'].elements)]
        }
      ...
      const retentionEntity = m.definitions['sap.ilm.RetentionService' + '.' + entityName] ?? JSON.parse(JSON.stringify(def));
      ...
      if (entityAlreadyExposed) {
        ...
        if (!_getBlockingDateField(retentionEntity.elements)) {
          retentionEntity.elements[_getBlockingDateField(m.definitions['sap.ilm.blocking'].elements)] = m.definitions['sap.ilm.blocking'].elements[_getBlockingDateField(m.definitions['sap.ilm.blocking'].elements)]
          if (!query.columns.includes('*') && !query.columns.some(c => c.ref && c.ref[0] === _getBlockingDateField(queryTarget.elements))) {
            query.columns.push({ ref: [_getBlockingDateField(queryTarget.elements)], as: _getBlockingDateField(m.definitions['sap.ilm.blocking'].elements) })
          }
        }
        if (!_getEndOfRetentionField(retentionEntity.elements)) {
          retentionEntity.elements[_getEndOfRetentionField(m.definitions['sap.ilm.destruction'].elements)] = m.definitions['sap.ilm.destruction'].elements[_getEndOfRetentionField(m.definitions['sap.ilm.destruction'].elements)]
          if (!query.columns.includes('*') && !query.columns.some(c => c.ref && c.ref[0] === _getEndOfRetentionField(queryTarget.elements))) {
            query.columns.push({ ref: [_getEndOfRetentionField(queryTarget.elements)], as: _getEndOfRetentionField(m.definitions['sap.ilm.destruction'].elements) })
          }
        }
      } else {
  ...
  for (let each in m.definitions) {
    ...
    if (each.startsWith('sap.ilm.RetentionService') && autoFillDPIRetentionSrv) {
      exposeAutoexposedEntities(def, 'sap.ilm.RetentionService', RetentionEntities, m, { exposeEntities: false })
  ...
  LOG.DEBUG && LOG.debug('DPI Information Model after modification by DPI plugin: ', Object.keys(m.definitions).filter(k => k === 'sap.ilm.InformationService').reduce((acc, key) => {
    acc[key] = m.definitions[key]
    return acc;
  }, {}))
  LOG.DEBUG && LOG.debug('DPI Retention Model after modification by DPI plugin: ', Object.keys(m.definitions).filter(k => k === 'sap.ilm.RetentionService').reduce((acc, key) => {
    acc[key] = m.definitions[key]
    return acc;
  }, {}))
