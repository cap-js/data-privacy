...
const { _getBlockingDateField, _getEndOfRetentionField, ensureAspectField } = require("../utils");
...
module.exports = function enhanceModel(m) {
  ...
  for (const artefactName in inferredCSN.definitions) {
    ...
    if (
      ...
      if (_getBlockingDateField(baseILMEntity.elements)) {
        ensureAspectField(entity, _getBlockingDateField, "sap.ilm.blocking", inferredCSN);
      }
      if (_getEndOfRetentionField(baseILMEntity.elements)) {
        ensureAspectField(entity, _getEndOfRetentionField, "sap.ilm.destruction", inferredCSN);
      }
      continue;
    }
