...
module.exports = function enhanceModel(m) {
  ...
  for (const each in m.definitions) {
    ...
    if (autoFillDPIInformationSrv) {
      ...
      const entityAlreadyExposed = !!m.definitions['DPIInformationService' + '.' + entityName];
      const informationEntity = m.definitions['DPIInformationService' + '.' + entityName] ?? JSON.parse(JSON.stringify(def));
      if (!entityAlreadyExposed) {
        ...
        m.definitions['DPIInformationService' + '.' + entityName] = informationEntity;
      }
      PDMEntities[each] = 'DPIInformationService' + '.' + entityName
      //This adds the compositions for DPIInformation
      const composedEntities = exposeCompositionsWithRewrites(each, informationEntity, fields, m, undefined, PDMEntities);
    ...
    if (autoFillDPIRetentionSrv) {
      ...
      const entityAlreadyExposed = !!m.definitions['DPIRetentionService' + '.' + entityName];
      const retentionEntity = m.definitions['DPIRetentionService' + '.' + entityName] ?? JSON.parse(JSON.stringify(def));
      ...
      if (entityAlreadyExposed) {
        ...
        m.definitions['DPIRetentionService' + '.' + entityName] = retentionEntity;
      }
      DRMEntities[each] = 'DPIRetentionService' + '.' + entityName
      //For retention compositions can be added without modifying columns
      exposeCompositions(retentionEntity, 'DPIRetentionService', DRMEntities, m, {exposeEntities: false});

      enhanceModelWithValueLists('DPIRetentionService' + '.' + entityName, retentionEntity, m);
    }
  }
  ...
  for (let each in m.definitions) {
    ...
    if (each.startsWith('DPIRetentionService') && autoFillDPIRetentionSrv) {
      exposeAutoexposedEntities(def, 'DPIRetentionService', DRMEntities, m, {exposeEntities: false})
  ...
  for (let each in m.definitions) {
    let def = m.definitions[each];
    if (each.startsWith('DPIRetentionService') && autoFillDPIRetentionSrv) {
      fixRelationTarget(def, DRMEntities, 'DPIRetentionService', m);
    } else if (each.startsWith('DPIInformationService') && autoFillDPIInformationSrv) {
      fixRelationTarget(def, PDMEntities, 'DPIInformationService', m);
    }
  }
