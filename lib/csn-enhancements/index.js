...
} = require("./retention/exposeEntity");
const {
...
module.exports = function enhanceModel(m) {
  ...
  for (const artefactName in inferredCSN.definitions) {
    ...
    // Skip already-exposed entities — blocking/destruction fields are propagated
    // recursively in exposeRetentionEntity via propagateAspectToProjections.
    // Join views are handled in a separate pass after all entities are enhanced.
    if (
