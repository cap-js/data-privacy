...
module.exports = function enhanceModel(m) {
  ...
  for (let each of Object.values(RetentionEntities)) {
    let def = m.definitions[each];
    // DataSubjects do not have OrgAttributes / SelectionCriteria / Conditions
    if (def['@PersonalData.EntitySemantics'] === 'Other') {
      redirectAssociationsToServiceIfPossible(def, 'sap.ilm.RetentionService', m);
      enhanceModelWithValueLists(each, def, m);
    }
  }
...
function redirectAssociationsToServiceIfPossible(def, serviceName, model) {
  PerElement: for (const ele in def.elements) {
    ...
    for (const e in model.definitions) {
      ...
      do {
        const sourceEntity = currentEntity.query
          ? currentEntity.query.SELECT.from.ref[0]
          : currentEntity.projection?.from?.ref[0];
