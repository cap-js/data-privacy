  ...
  for (const artefactName in inferredCSN.definitions) {
    ...
    if (
      ...
      if (
        ...
        const blockingField = _getBlockingDateField(baseILMEntity.elements);
        if (isUnion) {
          for (const arg of entity.query.SET.args) {
            arg.SELECT.columns ??= ["*"];
            if (
              !arg.SELECT.columns.includes("*") &&
              !arg.SELECT.columns.some((c) => c.ref && c.ref[0] === blockingField)
            ) {
              arg.SELECT.columns.push({ ref: [blockingField] });
            }
          }
        } else {
          ...
          if (query) {
            ...
            const queryTarget = sourceEntityName
              ? inferredCSN.definitions[sourceEntityName]
              : undefined;
            query.columns ??= ["*"];
            if (
              ...
              !query.columns.some(
                (c) =>
                  (c.ref && c.ref[0] === _getBlockingDateField(queryTarget?.elements)) ||
                  c.as === blockingField
              )
            ) {
              if (isJoin) {
                query.columns.push({
                  func: "min",
                  args: [{ ref: [blockingField] }],
                  as: blockingField
                });
              } else {
                query.columns.push({ ref: [blockingField] });
              }
            }
          }
        }
      ...
      if (
        ...
        const retentionField = _getEndOfRetentionField(baseILMEntity.elements);
        if (isUnion) {
          for (const arg of entity.query.SET.args) {
            arg.SELECT.columns ??= ["*"];
            if (
              !arg.SELECT.columns.includes("*") &&
              !arg.SELECT.columns.some((c) => c.ref && c.ref[0] === retentionField)
            ) {
              arg.SELECT.columns.push({ ref: [retentionField] });
            }
          }
        } else {
          const query = entity.query?.SELECT ?? entity.projection;
          if (query) {
            ...
            if (
              ...
              !query.columns.some(
                (c) =>
                  (c.ref && c.ref[0] === _getEndOfRetentionField(queryTarget?.elements)) ||
                  c.as === retentionField
              )
            ) {
              if (isJoin) {
