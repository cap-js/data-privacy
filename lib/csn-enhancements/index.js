...
const { _getOrgAttributeIDField, _getDataSubjectIDField, _getEndOfBusinessDateField, _getBlockingDateField, _getEndOfRetentionField } = require('../utils');
const enhanceAnnotations = require('./enhanceAnnotations');
const enhanceModelWithValueLists = require('./enhanceModelWithValueLists');
const { assignInformationAnnotations, exposeCompositionsWithRewrites } = require('./enhanceWithDPIInformation');
...
module.exports = function enhanceModel(m) {
  ...
  const _enhanced = 'sap.dpp.enhanced'
  if (m.meta?.[_enhanced] || m.definitions['sap.dpp.RetentionService']?.['@sap.dpp.model.enhanced']) return // already enhanced
  ...
  //const dpInformationModelPath = path.join(cds.root, cds.env.requires['sap.dpp.InformationService'].model.startsWith('.') ? '' : 'node_modules', cds.env.requires['sap.dpp.InformationService'].model + '.cds');
  //const dpRetentionModelPath = path.join(cds.root, cds.env.requires['sap.dpp.RetentionService'].model.startsWith('.') ? '' : 'node_modules', cds.env.requires['sap.dpp.RetentionService'].model + '.cds');
  const autoFillDPIInformationSrv = cds.env.requires['sap.dpp.InformationService'].model === '@sap/cds-dpi/srv/DPIInformation';
  const autoFillDPIRetentionSrv = cds.env.requires['sap.dpp.RetentionService'].model.startsWith('@sap/cds-dpi/srv/');
  ...
  LOG.debug('Existing sap.dpp.RetentionService: ', m.definitions['sap.dpp.RetentionService'])
  LOG.debug('Existing sap.dpp.InformationService: ', m.definitions['sap.dpp.InformationService'])

  for (const each in m.definitions) {
    const entity = getLowestILMObjectInProjectionHierarchy(each, m);
    const def = m.definitions[entity];
    if (def.kind !== 'entity' || !def['@PersonalData.EntitySemantics']) {
      continue;
    }
    if (entity in RetentionEntities || entity in InformationEntities || entity.startsWith('sap.dpp.RetentionService') || entity.startsWith('sap.dpp.InformationService')) {
      const originalEntity = m.definitions[each]
      if (_getBlockingDateField(def.elements)) {
        originalEntity.elements[_getBlockingDateField(m.definitions['sap.dpp.dppFlags'].elements)] = m.definitions['sap.dpp.dppFlags'].elements[_getBlockingDateField(m.definitions['sap.dpp.dppFlags'].elements)]
        const query = originalEntity.query?.SELECT ?? originalEntity.projection;
        if (query?.columns && !query.columns.some(c => c === '*')) {
          query.columns.push({ ref: [_getBlockingDateField(def.elements)] });
        }
      }
      if (_getEndOfRetentionField(def.elements)) {
        originalEntity.elements[_getEndOfRetentionField(m.definitions['sap.dpp.dppFlags'].elements)] = m.definitions['sap.dpp.dppFlags'].elements[_getEndOfRetentionField(m.definitions['sap.dpp.dppFlags'].elements)]
        const query = originalEntity.query?.SELECT ?? originalEntity.projection;
        if (query?.columns && !query.columns.some(c => c === '*')) {
          query.columns.push({ ref: [_getEndOfRetentionField(def.elements)] });
        }
      }
      if (def.elements['dppIsBlocked']) {
        Object.assign(originalEntity.elements, m.definitions['sap.dpp.dppCompositionFlags'].elements);
        const query = originalEntity.query?.SELECT ?? originalEntity.projection;
        if (query?.columns && !query.columns.some(c => c === '*')) {
          query.columns.push({ ref: ['dppIsBlocked'] });
        }
      }
      continue;
    }
    ...
    if (autoFillDPIInformationSrv) {
      ...
      const informationEntity = m.definitions['sap.dpp.InformationService' + '.' + entityName] ?? JSON.parse(JSON.stringify(def));
      ...
      InformationEntities[each] = 'sap.dpp.InformationService' + '.' + entityName
    ...
    if (autoFillDPIRetentionSrv) {
      ...
      if (!def.query && !def.projection) {
        if (!_getBlockingDateField(def.elements) && !_getEndOfRetentionField(def.elements)) {
          Object.assign(def.elements, m.definitions['sap.dpp.dppFlags'].elements);
          def.includes ??= []
          def.includes.push('sap.dpp.dppFlags');
        } else if (!_getBlockingDateField(def.elements)) {
          def.elements[_getBlockingDateField(m.definitions['sap.dpp.dppFlags'].elements)] = m.definitions['sap.dpp.dppFlags'].elements[_getBlockingDateField(m.definitions['sap.dpp.dppFlags'].elements)]
        } else if (!_getEndOfRetentionField(def.elements)) {
          def.elements[_getEndOfRetentionField(m.definitions['sap.dpp.dppFlags'].elements)] = m.definitions['sap.dpp.dppFlags'].elements[_getEndOfRetentionField(m.definitions['sap.dpp.dppFlags'].elements)]
        }
      ...
      const entityAlreadyExposed = !!m.definitions['sap.dpp.RetentionService' + '.' + entityName];
      const retentionEntity = m.definitions['sap.dpp.RetentionService' + '.' + entityName] ?? JSON.parse(JSON.stringify(def));
      retentionEntity['@cds.api.ignore'] = true;
      retentionEntity['@requires'] = 'InvalidRoleSoEntitiesCannotBeAccessedViaAPI';

      if (entityAlreadyExposed) {
        Object.assign(retentionEntity.elements, m.definitions['sap.dpp.dppFlags'].elements);
        const query = retentionEntity.query?.SELECT ?? retentionEntity.projection;
        query.columns ??= ['*'];
        query.columns.push({ ref: ['dppBlockingDate'] }, { ref: ['dppEarliestDestructionDate'] })
      } else {
        ...
        m.definitions['sap.dpp.RetentionService' + '.' + entityName] = retentionEntity;
      }
      RetentionEntities[each] = 'sap.dpp.RetentionService' + '.' + entityName
      //For retention compositions can be added without modifying columns
      // exposeCompositions(retentionEntity, 'sap.dpp.RetentionService', RetentionEntities, m, { exposeEntities: false });
      const composedEntities = exposeCompositionsWithRewrites(each, retentionEntity, m, {
        dsFields: {},
        redirectForParent: undefined,
        entities: RetentionEntities,
        dppServiceName: 'sap.dpp.RetentionService',
      ...
      if (retentionEntity['@PersonalData.EntitySemantics'] === 'Other') {
        enhanceModelWithValueLists('sap.dpp.RetentionService' + '.' + entityName, retentionEntity, m);
      }
    }
  }
  ...
  for (let each in m.definitions) {
    ...
    if (each.startsWith('sap.dpp.RetentionService') && autoFillDPIRetentionSrv) {
      exposeAutoexposedEntities(def, 'sap.dpp.RetentionService', RetentionEntities, m, { exposeEntities: false })
    } else if (each.startsWith('sap.dpp.InformationService') && autoFillDPIInformationSrv) {
      exposeAutoexposedEntities(def, 'sap.dpp.InformationService', InformationEntities, m)
    }
  }
  ...
  for (let each in m.definitions) {
    let def = m.definitions[each];
    if (each.startsWith('sap.dpp.RetentionService') && autoFillDPIRetentionSrv) {
      fixRelationTarget(def, RetentionEntities, 'sap.dpp.RetentionService', m);
    } else if (each.startsWith('sap.dpp.InformationService') && autoFillDPIInformationSrv) {
      fixRelationTarget(def, InformationEntities, 'sap.dpp.InformationService', m);
    }
  }

  //TODO: Check if own Retention Model that functions are assigned if they are not given

  LOG.DEBUG && LOG.debug('DPI Information Model after modification by DPI plugin: ', Object.keys(m.definitions).filter(k => k === 'sap.dpp.InformationService').reduce((acc, key) => {
    acc[key] = m.definitions[key]
    return acc;
  }, {}))
  LOG.DEBUG && LOG.debug('DPI Retention Model after modification by DPI plugin: ', Object.keys(m.definitions).filter(k => k === 'sap.dpp.RetentionService').reduce((acc, key) => {
    acc[key] = m.definitions[key]
    return acc;
  }, {}))

  // REVISIT: Setting var on service is a workaround because CSN meta is not passed along compiles
  m.definitions['sap.dpp.RetentionService']['@sap.dpp.model.enhanced'] = true
    ...
    if (def.elements[ele].target && typeof exposedEntities[def.elements[ele].target] === 'string') {
      ...
      if (def.elements[ele].keys) {
        for (const key of def.elements[ele].keys) {
          ...
          def.elements[ele + '_' + key.ref.join('_')] = JSON.parse(JSON.stringify(model.definitions[def.elements[ele].target].elements[key.ref.join('_')]));
          Object.keys(def.elements[ele]).filter(prop => prop.startsWith('@')).forEach(anno => {
            def.elements[ele + '_' + key.ref.join('_')][anno] = def.elements[ele][anno];
          });
          const query = def.query?.SELECT ?? def.projection;
          query.columns ??= ['*'];
          query.columns.push({ ref: [ele].concat(key.ref), as: ele + '_' + key.ref.join('_') });
          if (query.columns.some(c => c.ref && c.ref.length === 1 && c.ref[0] === ele)) {
            const idx = query.columns.findIndex(c => c.ref && c.ref.length === 1 && c.ref[0] === ele);
...
function exposeCompositions(def, srvPrefix, exposedEntities, m, options = { exposeEntities: true }) {
  ...
  if (compositons.length > 0) {
    for (const [, comp] of compositons) {
      ...
      const newCompEntity = m.definitions[srvPrefix + '.' + compEntityName] ?? JSON.parse(JSON.stringify(m.definitions[comp.target]));
...
function exposeAutoexposedEntities(def, srvName, srvEntityMappings, m, options = { exposeEntities: true }) {
  for (const ele in def.elements) {
    ...
    if (def.elements[ele].target && !srvEntityMappings[def.elements[ele].target] && m.definitions[def.elements[ele].target]['@cds.autoexpose']) {
      ...
      exposeCompositions(m.definitions[srvName + '.' + compEntityName], srvName, srvEntityMappings, m, options);
    }
  }
}
...
function addBlockingFlagToCompositions(elements, m) {
  for (const ele in elements) {
    if (elements[ele].type === 'cds.Composition') {
      ...
      def.includes ??= []
      def.includes.push('sap.dpp.dppCompositionFlags');
      Object.assign(def.elements, m.definitions['sap.dpp.dppCompositionFlags'].elements);
      addBlockingFlagToCompositions(def.elements, m)
    }
  }
}