...
module.exports = function enhanceModel(m) {
  ...
  for (const each in m.definitions) {
    ...
    if (entity in RetentionEntities || entity in InformationEntities || def.kind !== 'entity' || !def['@PersonalData.EntitySemantics'] || entity.startsWith('sap.dpp.RetentionService') || entity.startsWith('sap.dpp.InformationService')) {
      continue;
    }
    ...
    if (autoFillDPIInformationSrv) {
      ...
      const composedEntities = exposeCompositionsWithRewrites(each, informationEntity, fields, m, undefined, InformationEntities);
