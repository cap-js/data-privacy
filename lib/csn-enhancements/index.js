...
module.exports = function enhanceModel(m) {
  ...
  for (const each in m.definitions) {
    ...
    if (autoFillDPIRetentionSrv) {
      ...
      enhanceModelWithValueLists('DPIRetentionService' + '.' + entityName, newRetentionEntity, m);
    }
  }
      ...
      if (def.elements[ele].keys) {
        for (const key of def.elements[ele].keys) {
          ...
          if (def.query) {
            ...
            def.query.SELECT.columns.push({ref: [ele].concat(key.ref), as: ele + '_' + key.ref.join('_')})
          } else if (def.projection) {
            def.projection.columns ??= ['*'];
            def.projection.columns.push({ref: [ele].concat(key.ref), as: ele + '_' + key.ref.join('_')})
          }
        }
      }
