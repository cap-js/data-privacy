...
function handleValueList(value, name, type = 'selection', o) {
  ...
  if (
    ...
    const targetEntity =
      ...
      effectiveModel[o.entityName].elements[
        effectiveModel[o.entityName].elements[name]._foreignKey4
      ]?._target;
    if (!targetEntity) {
      LOG.error(
...
function buildConditions(o) {
  for (const elementName in o.entity.elements) {
    ...
    if (
      ...
      element['@ILM.FieldSemantics'] === 'ProcessOrganizationAttribute'
    ) {
...
function buildOrganizationAttribute(o) {
  ...
  if (orgAttributes.length > 1) {
    return LOG.error(
      `${o.entityName} has multiple organization attributes configured (annotated with @PersonalData.FieldSemantics : 'DataControllerID' or @ILM.FieldSemantics : 'LineOrganizationAttribute'! Only one attribute is allowed!`,
    );
  }
  if (orgAttributes.length === 0) {
    return LOG.error(
      `${o.entityName} has no organization attributes configured (annotated with @PersonalData.FieldSemantics : 'DataControllerID' or @ILM.FieldSemantics : 'LineOrganizationAttribute'! One attribute is required to configure the entity as an ILMObject for Data Privacy Retention Management!`,
    );
  }
  ...
  for (const element in elements) {
    if (
      ...
      (elements[element]['@PersonalData.FieldSemantics'] === 'DataControllerID' ||
        elements[element]['@ILM.FieldSemantics'] === 'LineOrganizationAttribute') &&
