...
module.exports = function exposeRetentionEntity(name, def, exposedEntities, m) {
  ...
  if (!def.query && !def.projection) {
    ...
    // current is now the base entity (no query/projection)
    if (current && !current.query && !current.projection) {
      if (!_getBlockingDateField(current.elements) && !_getEndOfRetentionField(current.elements)) {
        Object.assign(current.elements, m.definitions["sap.ilm.blocking"].elements);
        Object.assign(current.elements, m.definitions["sap.ilm.destruction"].elements);
        current.includes ??= [];
        current.includes.push("sap.ilm.blocking", "sap.ilm.destruction");
      } else if (!_getBlockingDateField(current.elements)) {
        current.elements[_getBlockingDateField(m.definitions["sap.ilm.blocking"].elements)] =
          m.definitions["sap.ilm.blocking"].elements[
            _getBlockingDateField(m.definitions["sap.ilm.blocking"].elements)
          ];
      } else if (!_getEndOfRetentionField(current.elements)) {
        current.elements[
          _getEndOfRetentionField(m.definitions["sap.ilm.destruction"].elements)
        ] =
          m.definitions["sap.ilm.destruction"].elements[
      ...
      // Propagate blocking/destruction fields up through each projection
      for (const proj of chain.reverse()) {
        if (
          !_getBlockingDateField(proj.elements) &&
          _getBlockingDateField(current.elements)
        ) {
          proj.elements[_getBlockingDateField(m.definitions["sap.ilm.blocking"].elements)] =
            m.definitions["sap.ilm.blocking"].elements[
              _getBlockingDateField(m.definitions["sap.ilm.blocking"].elements)
            ];
        }
        if (
          !_getEndOfRetentionField(proj.elements) &&
          _getEndOfRetentionField(current.elements)
        ) {
          proj.elements[
            _getEndOfRetentionField(m.definitions["sap.ilm.destruction"].elements)
          ] =
  ...
  if (entityAlreadyExposed) {
    ...
    query.columns ??= ["*"];
    const sourceEntityName = _getSourceEntity(_getQueryFrom(retentionEntity));
