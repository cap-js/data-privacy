...
function propagateAspectFieldsToJoinViews(m, dpiServiceNames) {
  for (const name in m.definitions) {
    ...
    // Only propagate if any join/union participant has the field
    const hasBlockingSource = _anySourceHasField(entity, _getBlockingDateField, m);
    const hasDestructionSource = _anySourceHasField(entity, _getEndOfRetentionField, m);
    if (hasBlockingSource) {
      ensureAspectField(entity, _getBlockingDateField, "sap.ilm.blocking", m);
    }
    if (hasDestructionSource) {
...
/**
 * Check whether any join/union participant has the given aspect field.
 * For joins, walks all args recursively via _collectJoinFieldRefs.
 * For unions, checks each SET arg's source.
 ...
 * @returns {boolean}
 */
function _anySourceHasField(entity, fieldGetter, model) {
  // Union: check all SET args
  if (entity.query?.SET) {
    for (const arg of entity.query.SET.args) {
      const from = arg.SELECT?.from;
      if (!from) continue;
      if (from.join) {
        if (_collectJoinFieldRefs(from, fieldGetter, model).length > 0) return true;
      } else if (from.ref) {
        const source = model.definitions[from.ref[0]];
        if (source && fieldGetter(source.elements)) return true;
      }
    }
    return false;
  }
  // Join: walk all args
  const from = entity.query?.SELECT?.from;
  if (!from) return false;
  if (from.join) {
    return _collectJoinFieldRefs(from, fieldGetter, model).length > 0;
  }
  // Simple ref (shouldn't reach here given the filter in the caller, but safe)
  if (from.ref) {
    const source = model.definitions[from.ref[0]];
    return !!(source && fieldGetter(source.elements));
  }
  return false;
}
