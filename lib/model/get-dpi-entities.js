...
function getDPIentities(model, retentionServiceDef) {
    const iLMObjects = [], organizationAttributes = [], handlersToRegister = [], messages = [];

    const { translate, getTranslationKey } = translationUtils(model);
    const servicePath = retentionServiceDef.path ?? retentionServiceDef['@path'];
    ...
    function handleValueList(value, name, entityName, entity, type = 'selection') {
        if (value['@Common.ValueList.CollectionPath'] && value.type !== 'cds.Association' && value.type !== 'cds.Composition') {
          ...
          const targetEntity = 
            ...
            ?? model.definitions[`${retentionServiceDef.name}.${value['@Common.ValueList.CollectionPath']}`] 
          ...
          conditionE.name = `${retentionServiceDef.name}.${ConditionVHName}`;
          ...
          const conditionE = new cds.builtin.classes.entity(conditionEntity);
          conditionE.name = `${retentionServiceDef.name}.${VHName}`;
          handlersToRegister.push({
            event: 'READ',
            name: VHName,
            entity: conditionE,
    ...
    for(const eName in model.definitions) {
        ...
        if (entity['@PersonalData.EntitySemantics'] === 'Other' && entity['@cds.drm.rootEntity']) {
          ...
          if (!_getLegalEntityIDField(entity.elements)) {
            messages.push({
              message: `${entity.name} lacks a property annotated with @PersonalData.FieldSemantics : 'DataControllerID', which is required for the Data Privacy service to work properly.`,
              severity: 'ERROR'
            })
          }
          if (entity['@PersonalData.DataSubjectRole']['=']) {
    ...
    //REVISIT: Instead expect enum in case of dynamic roles on the property given
