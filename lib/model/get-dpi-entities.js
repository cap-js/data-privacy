...
function getDPIentities(model, drmSrv) {
    ...
    function handleValueList(value, name, entityName, entity, type = 'selection') {
        if (value['@Common.ValueList.CollectionPath'] && value.type !== 'cds.Association' && value.type !== 'cds.Composition') {
          ...
          const targetEntity = model.definitions[`${namespace}.${value['@Common.ValueList.CollectionPath']}`];
          if (!targetEntity) {
            messages.push({
    ...
    function buildOrganizationAttribute(entity) {
      ...
      const orgAttribute = {
        ...
        organizationAttributeDescription : translate(entity.elements[orgAttributeName]['@Common.Label']),
        organizationAttributeBaseURL : '~{srv-api/srv-url}',
        organizationAttributeValueHelpEndPoint : `${servicePath}/LegalEntity`
      }
      ...
      if (endPoint) { //Case of annotated Common.ValueList
        ...
        const VHName = `valueHelp_orgAttribute_${entity.name.replace('.', '_')}_${orgAttributeName}`;
        const element = entity.elements[orgAttributeName];
        let descriptionField = element['@Common.Text']?.['='];
        if (!descriptionField || typeof descriptionField !== 'string') { //Not a string with edmJson for example
          if (element._foreignKey4 && entity.elements[element._foreignKey4]) {
            //If description is not defined but it is a foreign key
            ...
            descriptionField = target.elements[keys[0]]['@Common.Text']?.['='] ?? target['@UI.HeaderInfo.Title.Value']?.['='] ?? orgAttributeName;
          } else {
            descriptionField = orgAttributeName;
          }
        }
        const conditionE = new cds.builtin.classes.entity(conditionEntity);
