 ...
 * @returns 
 */
module.exports = function dpiServiceGeneration(m) {
  const DRMEntities = {};
  const PDMEntities = {};
  ...
  for (const each in m.definitions) {
    ...
    if (entity in DRMEntities || entity in PDMEntities || def.kind !== 'entity' || !def['@PersonalData.EntitySemantics']) {
      continue;
    }

    //Skip datasubject details if they are a composition of data subject as the data subject will include them
    if (
    ...
    if (autoFillDPIInformationSrv) {
      ...
      //REVISIT: Check with containment if compositions are correctly exposed, likely composition entites need to be removed from model after they have been rewritten
...
function exposeAutoexposedEntities(def, srvName, srvEntityMappings, m) {
  for (const ele in def.elements) {
    if (def.elements[ele].target && !srvEntityMappings[def.elements[ele].target] && m.definitions[def.elements[ele].target]['@cds.autoexpose']) {
      ...
      exposeCompositions(newCompEntity, srvName, srvEntityMappings, m);
    }
  }
}
