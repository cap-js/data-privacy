...
function getOrgAttributes(elements) {
    const orgAttributes = [];
    for (const element in elements) {
        if (elements[element] && elements[element]['@PersonalData.FieldSemantics'] === 'LegalEntityID' && (elements[element].isAssociation || elements[element].type === 'cds.Association')) {
...
const _getDataSubjectEntities = (dsRole, srv) => {
    ...
    for (const entityName in srv.entities) {
        ...
        if (entity && (entity['@PersonalData.DataSubjectRole'] === dsRole || entity['@PersonalData.DataSubjectRole']?.['=']) && (entity['@PersonalData.EntitySemantics'] === 'DataSubjectDetails' || entity['@PersonalData.EntitySemantics'] === 'DataSubject')) {
...
const _getILMObjectEntitiesForDS = (dsRole, srv) => {
    const entities = []
    for (const entityName in srv.entities) {
        const entity = srv.entities[entityName]
...
const _getEntityForILMObject = (iLMObject, srv) => {
    const names = iLMObject.split('.')
    const name = names[names.length-1]
    return srv.entities[name]
}

const _getWholeObjectTree = (entity, dataSubjectRole) => {
    const columns = []
    columns.push('*');
    if (entity['@PersonalData.DataSubjectRole']['='] && dataSubjectRole) {
        columns.push({ref: entity['@PersonalData.DataSubjectRole']['='], as: 'dataSubjectRole'});
    }
    if (entity.compositions) {
        for (const [name, composition] of Object.entries(entity.compositions)) {
            columns.push({ref: [name], expand: _getWholeObjectTree(composition._target, dataSubjectRole)})
        }
    }
    return columns
} 
...
const _nullForeignKeysOnILMObject = (entity, whereClause) => {
    const nullForeignKeysUpdateQueries = []
    if (!cds.db) return []
    entity = _getRoot(entity);
    const model = cds.context?.model ?? cds.model;
    for (const def in model.definitions) {
        const definition = model.definitions[def]
