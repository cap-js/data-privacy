...
/**
 * Collect all table-aliased references to a blocking/destruction field from a JOIN's from clause.
 * Walks nested joins recursively. Returns array of { ref: [alias, fieldName] }.
 * @param {object} from - CSN from clause (may be join with args)
 * @param {Function} fieldGetter - e.g. _getBlockingDateField
 * @param {object} model - CSN model
 * @returns {Array} refs with table aliases
 */
function _collectJoinFieldRefs(from, fieldGetter, model) {
  const refs = [];
  if (!from) return refs;
  if (from.ref && from.as) {
    const entity = model.definitions[from.ref[0]];
    if (entity && fieldGetter(entity.elements)) {
      refs.push({ ref: [from.as, fieldGetter(entity.elements)] });
    }
  }
  if (from.args) {
    for (const arg of from.args) {
      refs.push(..._collectJoinFieldRefs(arg, fieldGetter, model));
    }
  }
  return refs;
}
...
/**
 * Build a CSN expression that computes the row-level minimum of multiple column references.
 * For a single ref, returns it directly. For two refs, builds a CASE WHEN expression.
 * For N > 2, nests recursively.
 * This is needed because SQL MIN() is an aggregate (across rows), but we need the
 * minimum across columns within the same row.
 * @param {Array} refs - Array of { ref: [alias, fieldName] }
 * @returns {object} CSN expression
 */
function _buildRowMin(refs) {
  if (refs.length === 1) return refs[0];
  // Reduce right: min(a, min(b, c)) = min(a, b, c)
  const right = refs.length === 2 ? refs[1] : _buildRowMin(refs.slice(1));
  const left = refs[0];
  return {
    xpr: [
      "case",
 ...
 * Handles simple projections, JOINs, and UNIONs (adds to each SET arg).
 * For JOINs with multiple sources having the field, uses MIN() wrapping a row-level
 * CASE expression to get the earliest date across all joined entities.
...
function ensureAspectField(entity, fieldGetter, aspectName, model) {
  ...
  const aspectElement = model.definitions[aspectName].elements[aspectFieldName];
  const isJoin = !!entity.query?.SELECT?.from?.join;
  const isUnion = !!entity.query?.SET;
  if (isJoin || isUnion) {
    // For joins/unions, omit explicit type — it is inferred from the query column.
    // Only copy annotations to avoid compiler errors on inferred type mismatch.
    const { type: _type, ...rest } = aspectElement;
    entity.elements[aspectFieldName] = { ...rest };
  } else {
    entity.elements[aspectFieldName] = { ...aspectElement };
  }
  ...
  if (isUnion) {
    for (const arg of entity.query.SET.args) {
      ...
      if (
        ...
        !arg.SELECT.columns.some(
          (c) => (c.ref && c.ref[c.ref.length - 1] === aspectFieldName) || c.as === aspectFieldName
        )
      ) {
        // If the union arg's FROM is a join, use aliased ref to avoid ambiguity
        if (arg.SELECT.from?.join) {
          const fieldRefs = _collectJoinFieldRefs(arg.SELECT.from, fieldGetter, model);
          if (fieldRefs.length > 1) {
            arg.SELECT.columns.push({ ..._buildRowMin(fieldRefs), as: aspectFieldName });
          } else if (fieldRefs.length === 1) {
            arg.SELECT.columns.push({ ref: fieldRefs[0].ref, as: aspectFieldName });
          } else {
    ...
    if (query) {
      ...
      if (
        ...
        if (isJoin) {
          const fieldRefs = _collectJoinFieldRefs(entity.query.SELECT.from, fieldGetter, model);
          if (fieldRefs.length > 1) {
            // Multiple sources — CASE expression picks earliest non-null per row
            query.columns.push({
              ..._buildRowMin(fieldRefs),
              as: aspectFieldName
            });
          } else if (fieldRefs.length === 1) {
            query.columns.push({
            ...
            query.columns.push({ ref: [aspectFieldName] });
          }
        } else {
          query.columns.push({
