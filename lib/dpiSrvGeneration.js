const cds = require('@sap/cds'), fs = require("fs"), path = require("path"), fullDPIService = require('./dpicsn.json').definitions
const serveLegalGrounds = require('./drm-handlers/transactional-data-discovery')
const LOG = cds.log('dpi-srv-gen')
...
const compCondition = (name, cmp, backLinkName) => {
  ...
  if (cmp.keys)
      return JSON.parse(JSON.stringify(cmp.keys)).reduce((acc, val) => {
          ...
          acc += `${name}.${val.ref[0]} = $self.${name}_${val.ref[0]}` //REVISIT if it has to be . or _
          return acc
      }, '')
}
const addCompositions = (fullName, def, dsFields, m, namespaceString, redirectForParent = false, DRMEntities) => {
  ...
  if (compositons.length > 0) {
    ...
    for (const [name, comp] of compositons) {
      ...
      const mixinRes = mixin()
      const columnsRes = columns() //columns has to run before annotations so the references of dsFields are correct
      result += `${annotations(entity, newDsFields)} `+
          `entity ${shortName} as select from ${target} mixin {${mixinRes}} into {${columnsRes}} ${excluding()};`
...
module.exports = function dpiServiceGeneration() {
    ...
    return m => {
        ...
        LOG.debug('Existing DRMService: ', m.definitions.DRMService)
        LOG.debug('Existing PDMService: ', m.definitions.PDMService)
        ...
        dpiModel.definitions.DRMService['@impl'] = !cds.build.register ? path.join(__dirname, './drm-service.js') : 'srv/drm-service.js'
        ...
        LOG.debug('Model after modification by DPI plugin: ', m)
