...
const { _getRoot, _getSourceEntity, _getQueryFrom } = require("../utils");

/**
 * Resolve an entity to its root base table, handling join/union views
 * by following the primary source recursively.
 */
function _resolveToRoot(entity, definitions) {
  const root = _getRoot(entity);
  if (root !== entity) return root;
  // _getRoot returned entity itself — it's a join/union or has no _target
  if (entity.query?.SELECT?.from?.join || entity.query?.SET) {
    const sourceName = _getSourceEntity(_getQueryFrom(entity));
    if (sourceName && definitions[sourceName]) {
      return _resolveToRoot(definitions[sourceName], definitions);
    }
  }
  return entity;
}
...
cds.compiler.to.hdi.migration = function (csn, options, beforeImage) {
  if (csn.definitions["sap.ilm.RetentionService"]) {
  ...
  if (csn.definitions["sap.ilm.RetentionService"]) {
    res.definitions = res.definitions.concat(generateHDIDppArtefacts(csn));
  }
  return res;
};

function generateHDIDppArtefacts(csn) {
  const hdiArtefacts = [];
  ...
  for (const name in csn.definitions) {
    ...
    const tableEntity = _resolveToRoot(entity, csn.definitions);
    ...
    if (
      ...
      const blockRef = effectiveCSN.definitions[name]._dpi.blockingDateReference;
      if (!blockRef) {
        LOG.error(
          `No blocking date field found for ${name}! Please open an issue for cap-js/data-privacy.`
        );
        continue;
      }
      let whereConditionBlocked = `(${blockRef} is null or ${blockRef} &gt; CURRENT_UTCTIMESTAMP)`;
      if (
        ...
        for (const scope of auditorScopes) {
          if (!/^\w+$/.test(scope)) {
            LOG.error(
              `Invalid auditor scope value "${scope}" on ${name}. Only (a-z, A-Z, 0-9, _) are allowed!`
            );
            continue;
          }
...
function enhanceModelForDBRestrictions(csn) {
  ...
  // Find all views depending on the iLMObject tables and append @sql.append: 'WITH STRUCTURED PRIVILEGE CHECK'
  for (const artefact in csn.definitions) {
    const entity = csn.definitions[artefact];
    if (entity.kind !== "entity") continue;
    const tableEntity = _resolveToRoot(entity, csn.definitions);
    if (
      tableEntity.name !== entity.name &&
