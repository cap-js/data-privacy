...
cds.compiler.to.hdi.migration = function (csn, options, beforeImage) {
  ...
  for (const name in csn.definitions) {
    ...
    if (
      ...
      !entity.name.startsWith('sap.ilm.InformationService') &&
      ...
      if (
        effectiveCSN.definitions[name]['@Common.Auditing.AuditorScopes'] ||
        effectiveCSN.definitions[name]._service?.['@Common.AuditingDefaults.AuditorScopes']
      ) {
        const auditorScopes =
          csn.definitions[name]['@Common.Auditing.AuditorScopes'] ||
          effectiveCSN.definitions[name]._service?.['@Common.AuditingDefaults.AuditorScopes'];
...
cds.on('served', async () => {
  ...
  if (
    ...
    db.before('*', (req) => {
      ...
      const auditorScopes = req.target['@Common.Auditing.AuditorScopes'] ||
        req.target._service?.['@Common.AuditingDefaults.AuditorScopes'] || ['Auditor'];
      ...
      if (
        ...
        (cds.env.requires.db.kind !== 'hana' &&
          req.target._service?.name !== 'sap.ilm.InformationService' &&
