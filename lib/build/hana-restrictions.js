...
cds.compiler.to.hdi.migration = function (csn, options, beforeImage) {
    ...
    const iLMObjects = csn.definitions['sap.dpp.RetentionService']._dpi.iLMObjects;
    const dataSubjects = csn.definitions['sap.dpp.RetentionService']._dpi.dataSubjects;
    ...
    const collectEntities = (name, parent, compositionName) => {
        ...
        iLMRelevantTables[root.name] = {
            path: parent ? iLMRelevantTables[_getRoot(csn.definitions[parent]).name].path.concat(compositionName) : [name],
            definition: root
        };
    ...
    const effectiveCSN = cds.reflect(JSON.parse(JSON.stringify(csn)))
    ...
    for (const name in csn.definitions) {
        ...
        if (tableEntity.name !== entity.name && iLMRelevantTables[tableEntity.name] && !entity.name.startsWith('sap.dpp.InformationService') && !entity.name.startsWith('sap.dpp.RetentionService')) {
            const entityName = entity.name.replaceAll('.', '_').toUpperCase()

            let whereConditionBlocked = csn.definitions[tableEntity.name].elements['dppIsBlocked']
                ? `(dppIsBlocked is null or dppIsBlocked = false)`
                : `(${effectiveCSN.definitions[name]._dpi.blockingDateReference} is null or ${effectiveCSN.definitions[name]._dpi.blockingDateReference} &gt; CURRENT_UTCTIMESTAMP)`
            if (effectiveCSN.definitions[name]['@Common.Auditing.AuditorScopes'] || effectiveCSN.definitions[name]._service?.['@Common.AuditingDefaults.AuditorScopes']) {
                const auditorScopes = csn.definitions[name]['@Common.Auditing.AuditorScopes'] || effectiveCSN.definitions[name]._service?.['@Common.AuditingDefaults.AuditorScopes'];
    ...
    res.definitions.push({
        name: `sap.dpp.NoDPPRestriction`,
        suffix: '.hdbanalyticprivilege',
        sql: `<?xml version="1.0" encoding="UTF-8"?>
            <Privilege:analyticPrivilege xmlns:Privilege="http://www.sap.com/ndb/BiModelPrivilege.ecore" id="sap.dpp.NoDPPRestriction" privilegeType="SQL_ANALYTIC_PRIVILEGE" schemaVersion="1.1">
    ...
    const blockedDataAccessRole = {
        role: {
            name: "sap.dpp.RestrictBlockedDataAccess",
    ...
    if (cds.env.requires['sap.dpp.RetentionService']?.tableRestrictions === 'db') {
    ...
    res.definitions.push({
        name: `sap.dpp.DPPNoRestrictions`,
        sql: JSON.stringify({
            role: {
                name: "sap.dpp.NoRestrictions",
                schema_analytic_privileges: [
                    {
                        privileges: ['sap.dpp.NoDPPRestriction']
                    }
                ]
            }
        }),
...
function enhanceModelForDBRestrictions(csn) {
    ...
    const iLMObjects = effectiveModel.definitions['sap.dpp.RetentionService']._dpi.iLMObjects;
    const dataSubjects = effectiveModel.definitions['sap.dpp.RetentionService']._dpi.dataSubjects;
    ...
    // Find all views depending on the iLMObject tables and append @sql.append: 'WITH STRUCTURED PRIVILEGE CHECK'
    for (const entity of csn.collect(d => d.kind === 'entity', d => d)) {
        const tableEntity = _getRoot(entity);
        if (tableEntity.name !== entity.name && iLMRelevantTables[tableEntity.name] && !entity.name.startsWith('sap.dpp.InformationService') && !entity.name.startsWith('sap.dpp.RetentionService')) {
...
cds.on('served', async () => {
    ...
    db.before('*', (req) => {
        req._tx.set({ ROLES: `${Object.keys(cds.context.user.roles).join(',') + ','}` })
    });
    db.after('*', (res, req) => {
        req._tx.set({ ROLES: '' })
    });
    ...
    if (cds.env.requires['sap.dpp.RetentionService']?.tableRestrictions === 'srv' || cds.env.requires.db.kind !== 'hana') {
        db.before('*', (req) => {
            if (!req.target) return; //BEGIN, …
            const auditorScopes = req.target['@Common.Auditing.AuditorScopes'] || req.target._service?.['@Common.AuditingDefaults.AuditorScopes'] || ['Auditor'];
            ...
            if (
                ...
                (cds.env.requires.db.kind !== 'hana' && req.target._service?.name !== 'sap.dpp.InformationService' && req.target._service?.name !== 'sap.dpp.RetentionService')
            ) {
                const model = cds.context?.model ?? cds.model
                if (model.definitions[req.target.name].elements['dppIsBlocked']) {
                    const query = retrieveLowestQuery(req.query)
                    const where = [{ref: ['dppIsBlocked']}, 'is', {val: null}, 'or', {ref: ['dppIsBlocked']}, '=', {val: false}]
                    query.where = query.where ? [{xpr: query.where}, 'and', {xpr : where}] : where
                } else if (req.target._dpi.blockingDateReference) {
                    const query = retrieveLowestQuery(req.query)
                    const where = [{ref: [req.target._dpi.blockingDateReference]}, 'is', {val: null}, 'or', {ref: [req.target._dpi.blockingDateReference]}, '>', {ref: ['$now']}]
                    query.where = query.where ? [{xpr: query.where}, 'and', {xpr : where}] : where
                }
            }
        })
    }
    if (cds.env.requires['sap.dpp.RetentionService']?.tableRestrictions === 'db' && cds.env.requires.db.kind !== 'hana') {
        db.before('*', (req) => {
            if (!req.target) return; //BEGIN, …
            const auditorScopes = req.target['@Common.Auditing.AuditorScopes'] ?? req.target._service?.['@Common.AuditingDefaults.AuditorScopes'] ?? ['Auditor'];
            if (auditorScopes.some(scope => req.user.is(scope)) || req.user._is_privileged) return;
            ...
            if (req.event === 'READ' && !req.target.query && !req.target.projection) {
                if (req.target._dpi.isILMObject || req.target._dpi.dataSubject) {
                    return req.reject(401, `Entity ${req.target.name} is either an ILMObject or a DataSubject and as such its table cannot be directly accessed!`)
                }
            }
        }) 
    }
