...
cds.compiler.to.hdi.migration = function (csn, options, beforeImage) {
    ...
    for (const name in csn.definitions) {
        ...
        if (tableEntity.name !== entity.name && iLMRelevantTables[tableEntity.name] && !entity.name.startsWith('sap.ilm.InformationService') && !entity.name.startsWith('sap.ilm.RetentionService')) {
...
cds.on('served', async () => {
    ...
    if (cds.env.requires['sap.ilm.RetentionService']?.tableRestrictions === 'srv' || cds.env.requires.db.kind !== 'hana') {
        db.before('*', (req) => {
            ...
            if (
                ...
                (cds.env.requires.db.kind !== 'hana' && req.target._service?.name !== 'sap.ilm.InformationService' && req.target._service?.name !== 'sap.ilm.RetentionService')
            ) {
    ...
    if (cds.env.requires['sap.ilm.RetentionService']?.tableRestrictions === 'db' && cds.env.requires.db.kind !== 'hana') {
        db.before('*', (req) => {
            if (!req.target) return; //BEGIN, …
            const auditorScopes = req.target['@Common.Auditing.AuditorScopes'] ?? req.target._service?.['@Common.AuditingDefaults.AuditorScopes'] ?? ['Auditor'];
            if (auditorScopes.some(scope => req.user.is(scope)) || req.user._is_privileged) return;
