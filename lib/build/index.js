...
module.exports = class DPIPlugin extends cds.build.Plugin {
  ...
  async updateRetentionConfig(csn) {
    const retentionEnv = cds.env.requires?.["sap.ilm.RetentionService"];
    const deploymentDescriptor =
      retentionEnv?.deploymentdescriptor ?? retentionEnv?.deploymentDescriptor ?? "mta.yaml";
    const resolvedDescriptorPath = path.isAbsolute(deploymentDescriptor)
      ? deploymentDescriptor
      : path.join(cds.root, deploymentDescriptor);

    const project = readProject();
    if (fsSync.existsSync(resolvedDescriptorPath)) {
      const mta = cds.parse.yaml(
        await fs.readFile(resolvedDescriptorPath, {
          encoding: "utf-8"
        })
      );
      ...
      if (dpiRetentionInstance) {
        ...
        if (!hasIEoP) {
          throw new Error("To use iEoP configuration, at least one entity with @PersonalData.EntitySemantics 'DataSubject' must have the @ODM.entityName and @ODM.oid annotations defined.");
        }
  ...
  formatDataSubjectForConfig(role, entity, csn) {
    ...
    const formattedDataSubject = {
      ...
      dataSubjectUnblockingEndPoint: `${servicePath}/order-management/dataSubjectUnblocking`,
      iEoPObjectType: entity['@ODM.entityName']
