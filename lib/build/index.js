...
module.exports = class DPIPlugin extends cds.build.Plugin {
  ...
  async updateRetentionConfig(csn) {
    ...
    if (hasMta) {
      ...
      if (dpiRetentionInstance) {
        ...
        const dataSubjects = Object.keys(csn.definitions['sap.ilm.RetentionService']._dpi.dataSubjects)
        ...
        const orgAttributes = Object.keys(csn.definitions).filter(n => n.startsWith('sap.ilm.RetentionService.valueHelp_orgAttribute'))
        ...
        const resources = { in: 'resources', where: { 'name': dpiRetentionInstance.name } }
        const dataSubjectRoleAdditions = (dataSubjectRoles ?? []).map(dsr => ({ in: 'parameters.config.dataPrivacyConfiguration.retentionConfiguration.dataSubjectRoles', where: { dataSubjectRoleName: dsr.dataSubjectRoleName} }))
        const organizationAttributeAdditions = (organizationAttributes ?? []).map(orgAttr => ({ in: 'parameters.config.dataPrivacyConfiguration.retentionConfiguration.organizationAttributes', where: { organizationAttributeName: orgAttr.organizationAttributeName} }))
        await merge(
          {
            "resources": [
              {
                  ...
                  "config": {
                    "dataPrivacyConfiguration": {
                      "retentionConfiguration": {
                        ...
                        ...(organizationAttributes?.length ? { "organizationAttributes": organizationAttributes } : {})
                      }
                    }
                  }
                }
        ...
        ).into('mta.yaml', { project, additions: [resources, ...dataSubjectRoleAdditions, ...organizationAttributeAdditions] })
      }
  ...
  formatDataSubjectForConfig(role, entity, csn) {
    const servicePath = csn.definitions['sap.ilm.RetentionService'].path ?? csn.definitions['sap.ilm.RetentionService']['@path'];
