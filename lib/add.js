const cds = require('@sap/cds-dk') //> load from cds-dk
const { getDPIentities } = require('./model/get-dpi-entities')
const { readProject, merge, registries } = cds.add 
const { srv4 } = registries.mta 

module.exports = class extends cds.add.Plugin {
  async run() {}
  async combine() { 
    const project = readProject() 
    const { hasMta, srvPath } = project 
    const models = await cds.load(cds.env.folders.srv);
    const csn = cds.compile.to.csn(models);
    const result = getDPIentities(csn, csn.definitions.DRMService ?? {name: 'DRMService', path: '/drm'});
    const dpConfig = {
      dataPrivacyConfiguration: {
        retentionConfiguration: {
          applicationConfiguration: {
            applicationType : 'TransactionMaster',
            simulationSupportedForArchiving : true,
            simulationSupportedForDestruction : true,
            iLMObjectDiscoveryEndPoint: "/drm/iLMObjects"
          },
          dataSubjectRoles: result.dataSubjectRoles,
          organizationAttributes: result.organizationAttributes
        }
      }
    }
    const additions = [{ in: 'dataPrivacyConfiguration' }]
    await merge(dpConfig).into('dpi-retention.json', { project, additions })

    if (hasMta) { 
      const srv = srv4(srvPath) // Node.js or Java server module
      const dpiInfo = { in: 'resources', 
        where: { 
          'parameters.service': 'data-privacy-integration-service',
          'parameters.config.dataPrivacyConfiguration.configType': 'information'
        } 
      } 
      const dpiRetention = { in: 'resources', 
        where: {
          'parameters.service': 'data-privacy-integration-service',
          'parameters.config.dataPrivacyConfiguration.configType': 'retention'
        } 
      }
      const xsuaa = { in: 'resources', 
        where: { 'parameters.service': 'xsuaa' } 
      }
      await merge(__dirname, 'add/mta.yaml.hbs').into('mta.yaml', { 
        project, // for Mustache replacements
        additions: [srv, dpiInfo, xsuaa], 
        relationships: [{ 
            insert: [dpiInfo, 'name'], 
            into: [srv, 'requires', 'name'] 
        }] 
      })
      //Two merge functions needed because relationships even if an array, can only handle the first relationship for a into target and not multiple ones
      await merge(__dirname, 'add/mta.yaml.hbs').into('mta.yaml', { 
        project, // for Mustache replacements
        additions: [srv, dpiRetention, xsuaa], 
        relationships: [{ 
          insert: [dpiRetention, 'name'], 
          into: [srv, 'requires', 'name'] 
        }] 
      }) 
    } 
    // if (hasHelm) {
    //  ...
    // if (hasMultitenancy) {
    //  ...
  }
}