...
if (mode === "revert") {
  ...
  // Revert incidents-mgmt dependency back to file reference
  ...
  // Revert sidecar dependency back to file reference
  const sidecarPkg = path.resolve(appDir, "mtx/sidecar/package.json");
  const sidecarPkgContent = JSON.parse(fs.readFileSync(sidecarPkg, "utf8"));
  sidecarPkgContent.dependencies["@cap-js/data-privacy"] = "file:../../../../";
  fs.writeFileSync(sidecarPkg, JSON.stringify(sidecarPkgContent, null, 2) + "\n");
