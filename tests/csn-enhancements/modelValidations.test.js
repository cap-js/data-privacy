...
describe('Model validations to ensure a working model for DPI', () => {
  ...
  test('Error if a property has @ILM.FieldSemantics : LineOrg but conflicting PersonalData.FieldSemantics annotation', async () => {
    ...
    expect(log.output.length).toBeGreaterThan(0);
    expect(log.output).toContain(
      `is annotated with @ILM.FieldSemantics : 'LineOrganizationAttribute' but also a conflicting @PersonalData.FieldSemantics annotation! Please remove either of them!`,
    );
  });
  ...
  test('Error if a property has @ILM.FieldSemantics : ProcessOrg but conflicting PersonalData.FieldSemantics annotation', async () => {
    ...
    expect(log.output.length).toBeGreaterThan(0);
    expect(log.output).toContain(
      `is annotated with @ILM.FieldSemantics : 'ProcessOrganizationAttribute' but also a conflicting @PersonalData.FieldSemantics annotation! Please remove either of them!`,
    );
  });
  ...
  describe('Data type validations for field semantics', () => {
    ...
    test('ILM.FieldSemantics are checked for type correctness', async () => {
      ...
      expect(log.output).toContain(
        `The data type (${dataType}) of legalEntity of ${entityName} is not supported for @ILM.FieldSemantics : 'LineOrganizationAttribute'! Unsupported data types: cds.LargeBinary, cds.Binary, cds.Date, cds.DateTime, cds.Timestamp, cds.Map, cds.LargeString`,
      );
      expect(log.output).toContain(
        `The data type (${dataType}) of purpose of ${entityName} is not supported for @ILM.FieldSemantics : 'ProcessOrganizationAttribute'! Unsupported data types: cds.LargeBinary, cds.Binary, cds.Date, cds.DateTime, cds.Timestamp, cds.Map, cds.LargeString`,
      );
    });
  });
